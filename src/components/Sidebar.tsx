  ...
  return (
    <div className='w-[16rem] min-w-[16rem] max-h-screen overflow-scroll  bg-neutral-50'>
      <div className='sticky top-0 bg-neutral-50 z-50 px-3 py-5'>
        <Username />
        <div role='button' className='flex truncate pl-5 pr-3 pb-3 pt-2 rounded-md  border border-gray-200  hover:bg-gray-200 mt-6' onClick={createNewLog}>
      ...
      <div className='grid gap-1 px-3 pb-5'>
        {data.map((log, idx) => (
          <LogTab key={log.id} {...log} onClick={() => selectLog(idx)} active={activeLog == idx} />
        ))}
      </div>
    </div>
  );
